...
#### Java内存模型:
     ...
     在本地进行修改,然后写回主内存,而不是直接在主存中进行读写.
     
     那么这就可能造成可见性问题:有2个线程都保存了主存的变量,当一个线程修改了它的本地变量,
     并写回了主内存,但是另一个线程仍然使用的是之前的值,此时就造成了数据的不一致.
     
     volatile关键字修饰的变量就可以解决可见性问题:被volatile修饰的变量,
     要求线程使用时都从主内存中读取
...
####垃圾回收算法：
  ...
  ##### 复制算法:
         ...
         根据GC角度来说就是:在新手代的eden空间和From Survivor空间经历过MinorGC后,
  ...
  ##### 标记-清除算法:
         标记出所有需要被回收的对象,然后统一回收所有被标记的对象也就是仍然存活的对象.
